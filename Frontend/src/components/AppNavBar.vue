...
<template>
  <nav class="fixed top-0 left-0 w-full z-50 bg-blue-900 text-white" style="height: var(--navbar-h)">
    <div class="max-w-6xl mx-auto h-full px-4 flex items-center justify-between gap-4">
      <!-- LEFT -->
      <RouterLink to="/propostas" class="font-semibold text-lg hover:opacity-90 whitespace-nowrap">
        {{ title }}
      </RouterLink>
...
<!-- Só Admin -->
    <RouterLink v-if="authStore.isAdmin.value" to="/alunos" class="hover:underline">
      Alunos
    </RouterLink>
        ...
        <div v-if="isAuthenticated" class="hidden md:block relative" data-user-menu>
          ...
          >
            ...
            <button
              ...
              role="menuitem"
              @click="() => { userMenuOpen = false; emit('logout'); }"
            >
              Sair
            </button>
          </div>
